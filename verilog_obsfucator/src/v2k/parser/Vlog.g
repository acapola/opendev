...
: 	(attribute_instance)* 
		module_keyword mid=module_identifier {getTree().addSymbol(mid);}
...
:	LPAREN (pd=port_declaration[true]    {lopd = getTree().addToList(lopd,pd);} 
...
port_declaration[boolean isOnlyDefn] returns [PortDeclaration pd]
{ pd=null;}
:	(attribute_instance)*
...
local_parameter_declaration returns [LocalParameterDecl lpd]
{ lpd=null; boolean isSigned=false; Range rng=null; 
  ListOf<ParamAssign> lopa=null; ParameterType pt=null;
}
:	(	"localparam" ("signed" {isSigned=true;})? (rng=range)? 
...
parameter_declaration returns [ParameterDeclaration pd]
{ pd=null; boolean isSigned=false; Range rng=null; 
  ListOf<ParamAssign> lopa=null; ParameterType pt=null;
}
:	(	"parameter" ("signed" {isSigned=true;})? (rng=range)? 
			lopa=list_of_param_assignments
...
inout_declaration[boolean isOnlyDefn] returns [PortDeclaration pd]
{ pd=null; int dir=LA(1); NetType type=null; Range rng=null;
  boolean isSigned=false;
  ListOf<? extends PortIdent> ids = null;
}
:	"inout" (type=net_type)? ("signed" {isSigned=true;})? 
		(rng=range)? ids=list_of_port_identifiers
	{ pd = getTree().portDecl(isOnlyDefn,dir,type,rng,null,false,isSigned,ids);}
;

input_declaration[boolean isOnlyDefn] returns [PortDeclaration pd]
{ pd=null; int dir=LA(1); NetType type=null; Range rng=null;
  boolean isSigned=false;
  ListOf<? extends PortIdent> ids = null;
}
:	"input" (type=net_type)? ("signed" {isSigned=true;})? 
		(rng=range)? ids=list_of_port_identifiers
	{ pd = getTree().portDecl(isOnlyDefn,dir,type,rng,null,false,isSigned,ids);}
;

output_declaration[boolean isOnlyDefn] returns [PortDeclaration pd]
{ pd=null; int dir=LA(1); NetType type=null; Range rng=null;
  boolean isReg=false; boolean isSigned=false;
  ListOf<? extends PortIdent> ids = null; OutputVarType ovt=null;
}
:	(	("output" output_variable_type)=> "output" ovt=output_variable_type 
			ids=list_of_variable_port_identifiers
	|	("output" ("reg")? ("signed")? (range)? 
				list_of_variable_port_identifiers)=>
			"output" ("reg" {isReg=true;})? ("signed" {isSigned=true;})? 
				(rng=range)? ids=list_of_variable_port_identifiers
	|	"output" (type=net_type)? ("signed" {isSigned=true;})? (rng=range)? 
...
net_declaration returns [NetDeclaration nd]
...
:	type=net_type ("vectored" | "scalared")? ("signed" {isSigned=true;})? 
...
:	"inout" tpt=task_port_type lop=list_of_port_identifiers
		{tfp = getTree().tfPortDecl(dir, tpt, lop);}
;

tf_input_declaration returns [TfPortDeclaration tfp]
{ tfp=null; int dir=LA(1); TaskPortType tpt=null; ListOf<PortIdent> lop=null;}
:	"input" tpt=task_port_type lop=list_of_port_identifiers
		{tfp = getTree().tfPortDecl(dir, tpt, lop);}
;

tf_output_declaration returns [TfPortDeclaration tfp]
{ tfp=null; int dir=LA(1); TaskPortType tpt=null; ListOf<PortIdent> lop=null;}
:	"output" tpt=task_port_type lop=list_of_port_identifiers
		{tfp = getTree().tfPortDecl(dir, tpt, lop);}
;
...
|	("reg" {isReg=true;})? ("signed" {isSigned=true;})? (rng=range)?
...
: 	(attribute_instance)* 
	...
	|	lpd=local_parameter_declaration SEMI
...
:	"generate" 
		(mi=module_or_generate_item {loi = getTree().addToList(loi, mi);})* 
		"endgenerate"
;
...
:	mi=module_or_generate_item	{gb = getTree().generateBlock(bi,mi);}
| 	"begin" (COLON bi=generate_block_identifier)? 
