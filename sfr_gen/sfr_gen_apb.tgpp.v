...
proc sfr_apb_module { sfrs {custom_clock 0}} {
	set clock PCLK
	if { $custom_clock } {
		set clock i_clk
...
``if {$hasHardwareUpdate} {``//inputs for SFRs updated by hardware``}
dict for {sfr_name sfr_props} [dict get $sfrs bitfields] {
	if {[is_container $sfr_props]} { #declare each bitfield separately
		dict for {bf_name bf_props} [dict get $sfr_props bitfields] {
...
always @* PSLVERR = 1'b0;
always @* PREADY  = 1'b1;
wire sfr_write_access = valid_access & PENABLE & PWRITE``if {$custom_clock} {`` & `$clock_en```}``;
``
...
``
wire sfr_read_access  = valid_access `$enable_guard`& ~PWRITE;
``if {$register_PRDATA} {``
...
foreach p [dict keys $::hw_read_sens_ports] {
	append sens_ports "${sep}sfr_${p}_read"
	set sep ","
	incr sens_ports_cnt
}
set sep ""
